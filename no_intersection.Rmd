...
#st_geometry(crop.category.tract) <- NULL
```
```{r}
crop.category.tract
```
...
# pesticide use in 2019

```{r}
#estimate.2019 <- subset(HighEstimate_AgPestUsebyCropGroup, Year == 2019)

#str(estimate.2019)
#cols = c(6:15);    
#estimate.2019[,cols] = apply(estimate.2019[,cols], 2, function(x) as.numeric(as.character(x)));

#estimate.2019$all <- rowSums(estimate.2019[ , 6:15], na.rm = T)
#sum(estimate.2019$all)

#estimate.2019.state <- estimate.2019 %>%
#  group_by(State_FIPS_code) %>% 
#  summarise(sum = sum(all)) #AREA_SQMI
...
# pesticide use in 2011
#estimate.2011 <- subset(HighEstimate_AgPestUsebyCropGroup, Year == 1992)

#str(estimate.2011)
#cols = c(6:15);    
#estimate.2011[,cols] = apply(estimate.2011[,cols], 2, function(x) as.numeric(as.character(x)));

#estimate.2011$all <- rowSums(estimate.2011[ , 6:15], na.rm = T)
#sum(estimate.2011$all)

#estimate.2011.state <- estimate.2011 %>%
#  group_by(State_FIPS_code) %>% 
#  summarise(sum = sum(all)) #AREA_SQMI
#"
...
```{r}
plot <- ggplot(data = oregon_map, aes(long, lat, group = group, fill = M_POV150)) +
  geom_polygon(color = "black", linewidth = 0.2) +
  coord_fixed(1.3) +
  scale_fill_viridis_c(name = "M_POV150", option = "viridis", na.value = "gray90") +
  theme_minimal() +
  labs(title = "Poverty Distribution in Oregon",
       subtitle = "Shaded by M_POV150")
plot
```
```{r}
...
```
```{r}
plot <- ggplot(data = oregon_map, aes(long, lat, group = group, fill = M_UNEMP)) +
  geom_polygon(color = "black", linewidth = 0.2) +
  coord_fixed(1.3) +
  scale_fill_viridis_c(name = "Unemployment", option = "viridis", na.value = "gray90") +
  theme_minimal() +
  labs(title = "Unemployment Distribution in Oregon",
       subtitle = "Shaded by M_UNEMP")
plot
```
